...
class PDFRenderMixin(object):
    """Class-based view Mixin for HTML templates rendered to PDF."""
    ...
    def get_pdf_template_name(self):
        if not self.pdf_template_name:
            raise ImproperlyConfigured(
                ...
                '{0}.get_pdf_template_name().'.format(
    ...
    def render_to_response(self, context, **response_kwargs):
        as_return = self.request.GET.get('as', None)
        if (as_return == 'pdf' or self.pdf_default_response_is_pdf) and not as_return == 'html':
...
class PDFTemplateView(PDFRenderMixin, TemplateView):
    ...
    def get_pdf_template_name(self):
        if not self.pdf_template_name and hasattr(self, 'template_name') and self.template_name:
